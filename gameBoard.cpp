...
void SlidingTiles::GameBoard::randomGame(const std::size_t emptyTiles) {
  ...
  throw std::runtime_error(
      "Something is terribly wrong. We rendered 200 game boards and got a "
...
auto SlidingTiles::GameBoard::pickEndTile(const sf::Vector2i &endPos)
    -> SlidingTiles::Tile {
  Tile endTile{};
...
void SlidingTiles::GameBoard::randomGameImpl(const std::size_t emptyTiles) {
  assert(emptyTiles > 0 &&
         emptyTiles <
             boardSize * boardSize -
                 2); // NOLINT
  ...
  for (auto i = 2 + emptyTiles; i < totalTiles; ++i) {
    const sf::Vector2i tilePos = positions[i];
    Tile tile{};
...
auto SlidingTiles::GameBoard::getAdjacentTilePosition(const Move &move)
  ...
  if (adjacentPosition.x < 0 || adjacentPosition.y < 0 ||
      adjacentPosition.x >= boardSize || adjacentPosition.y >= boardSize) {
...
auto SlidingTiles::GameBoard::getOutputPosition(const Move &move) noexcept(
  ...
  if ((move.startPosition.x < 0) || (move.startPosition.x >= boardSize) ||
    ...
    throw std::out_of_range(
        "getOutputPosition called with an invalid startPosition in the move");
  }
  auto type =
      getTile(move.startPosition.x, move.startPosition.y)->getTileType();
  ...
  if ((type == TileType::StartRight && move.startPosition.x < boardSize - 1) ||
      (type == TileType::Horizontal && move.startPosition.x < boardSize - 1 &&
       move.direction == Direction::GoRight) ||
      (type == TileType::BottomRight && move.startPosition.x < boardSize - 1 &&
       move.direction == Direction::GoUp)) {
    ++nextTile.x;
  } else if ((type == TileType::StartLeft &&
              move.startPosition.x < boardSize - 1 &&
              move.startPosition.x > 0) ||
             (type == TileType::Horizontal && move.startPosition.x > 0 &&
              move.direction == Direction::GoLeft) ||
             (type == TileType::LeftBottom && move.startPosition.x > 0 &&
              move.direction == Direction::GoUp) ||
             (type == TileType::LeftTop && move.startPosition.x > 0 &&
              move.direction == Direction::GoDown)) {
    --nextTile.x;
  } else if ((type == TileType::StartTop && move.startPosition.y > 0) ||
             (type == TileType::Vertical && move.startPosition.y > 0 &&
              move.direction == Direction::GoUp) ||
             (type == TileType::LeftTop && move.startPosition.y > 0 &&
              move.direction == Direction::GoRight)) {
    --nextTile.y;
  } else if ((type == TileType::StartBottom &&
              move.startPosition.y < boardSize - 1) ||
             (type == TileType::Vertical &&
              move.startPosition.y < boardSize - 1 &&
              move.direction == Direction::GoDown) ||
             (type == TileType::LeftBottom &&
              move.startPosition.y < boardSize - 1 &&
              move.direction == Direction::GoRight)) {
    ++nextTile.y;
  } else {
    if (type == TileType::BottomRight && move.startPosition.y < boardSize - 1 &&
        move.direction == Direction::GoLeft) {
      ++nextTile.y;
    } else {
      if (type == TileType::TopRight && move.startPosition.y > 0 &&
          move.direction == Direction::GoLeft) {
        --nextTile.y;
      } else {
        if (type == TileType::TopRight &&
            move.startPosition.x < boardSize - 1 &&
            move.direction == Direction::GoDown) {
          ++nextTile.x;
        } else {
          if ((type == TileType::EndBottom &&
               move.direction == Direction::GoUp) ||
              (type == TileType::EndLeft &&
               move.direction == Direction::GoRight) ||
              (type == TileType::EndRight &&
...
auto SlidingTiles::GameBoard::getOutputMove(const Move &move)
  ...
  Move outputMove{getOutputPosition(move),
                  startTile->outputDirection(move.direction)};
  return outputMove;
}
...
auto SlidingTiles::GameBoard::isSolved() -> std::vector<sf::Vector2i> {
  ...
  Move move{startTile->getTilePosition(),
            startTile->outputDirection(Direction::Unknown)};
  int count{0};
