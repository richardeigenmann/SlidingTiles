...
SlidingTiles::RenderingSingleton::RenderingSingleton()
    : window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT, COLOR_DEPTH), TITLE) {
  window.setFramerateLimit(FRAME_RATE_LIMIT);
}

const int SlidingTiles::RenderingSingleton::tileSize;

auto SlidingTiles::RenderingSingleton::calculateCoordinates(
    const sf::Vector2i &coord) const -> sf::Vector2i {
  sf::Vector2i coordinates = gridZeroZero;
  coordinates.x += coord.x * tileSize;
  coordinates.y += coord.y * tileSize;
  return coordinates;
}

auto SlidingTiles::RenderingSingleton::findTile(sf::Vector2i mousePosition) const
    -> sf::Vector2i {
  const sf::Vector2i gridCoordinates = mousePosition - gridZeroZero;
