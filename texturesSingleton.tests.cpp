...
TEST(TexturesSingleton, ObjectCreation) { TexturesSingleton::getInstance(); }

TEST(TexturesSingleton, LoadTextures) {
  std::map<TileType, sf::Texture> textureMap =
      TexturesSingleton::getInstance().getTexturesMap();
...
TEST(TexturesSingleton, TestAllTextureSizes) {
  ...
  for (const auto &myPair : textureMap) {
    ...
    textureSize = emptyTexture.getSize();
    ASSERT_GT(textureSize.x, 0)
        << "The width of the texture for TileType::"
        << tileTypeToString(myPair.first) << " is too small";
    ASSERT_GT(textureSize.y, 0)
        << "The height of the texture for TileType::"
        << tileTypeToString(myPair.first) << " is too small";
  }
}
