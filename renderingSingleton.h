...
public:
     ...
     /**
     * @brief Saves the asset directory. Expects the argv[0] that invoked the program as the assets are expected to be relative to the program file.
     */
     void setAssetDir(const std::string &dir)
     {
          std::experimental::filesystem::path p{dir};
          assetDir = p.parent_path().string() + '/';
     }
...
private:
     ...
     /** 
         ...
         */
     std::string assetDir{"uninitialized"};
};
