...
TEST(Tile, outputDirectionStartRight) {
  ...
  SlidingTiles::Direction newDirection =
      tile.outputDirection(SlidingTiles::Direction::Unknown);
...
TEST(Tile, transition) {
  /*std::string game
     [SlidingTiles::GameBoard::boardSize][SlidingTiles::GameBoard::boardSize]{"├",
     "-", "-", "┐", "┣", "┐", " ", "|", "┌", "┘", " ", "|", "└", "-", "-",
     "┘"};*/
  std::wstring game{L"├--┐┣┐ |┌┘ |└--┘"};
  ...
  // ASSERT_FALSE(result) << "Transitioning should not be possible when
